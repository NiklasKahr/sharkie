class SuperJellyFish extends MovableObject {
    ...
    constructor(color, x, y, backAndForth, direction) {
        super().loadImage(SJF_IMAGES_SWIMMING[color][0]);
        this.loadImages(SJF_IMAGES_SWIMMING[color]);
        this.loadImages(SJF_IMAGES_DYING[color]);
        this.color = color;
    ...
    animate() {
        this.swimming = setInterval(() => {
            this.playAnimation(SJF_IMAGES_SWIMMING[this.color]);
        }, 1000 / 6);
    }
}